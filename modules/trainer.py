...
class Trainer(object):
    def __init__(self, args, net, train_loader, val_loader=None):
        ...
        self.optimizer = torch.optim.SGD(
            tuned_lrs, lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)

        self.scheduler = lr_scheduler.StepLR(
            self.optimizer, step_size=args.stepsize, gamma=args.gamma)
    ...
    def train(self, epoch):
        ...
        with tqdm(total=self.n_train, desc=f'Epoch {epoch + 1}/{self.max_epoch}', unit='img', disable=torch.distributed.get_rank()!=0) as pbar:
            for batch in self.train_loader: 
                ...
                # (self.n_dataset // (10 * self.batch_size)) == 0:
                    ...
                    if self.writer is not None:
                        ...
                        for tag, value in self.model.named_parameters():
                            ...
                            self.writer.add_histogram(
                                'weights/' + tag, value.data.cpu().numpy(), self.global_step)
                            self.writer.add_histogram(
                                'grads/' + tag, value.grad.data.cpu().numpy(), self.global_step)

                        self.writer.add_images(
                            'images', image, self.global_step)
                        self.writer.add_images(
                            'masks/true', label, self.global_step)
                        self.writer.add_images(
                            'masks/pred', outputs[-1] > 0.5, self.global_step)
        ...
        if self.tmp_dir is not None:
            self.save_state(epoch, save_path=join(
                self.tmp_dir, f'checkpoint_epoch{epoch+1}.pth'))
...
def tune_lrs(model, lr, weight_decay):
    ...
    bias_params = [param for name, param in list(
        model.named_parameters()) if name.find('bias') != -1]
    weight_params = [param for name, param in list(
        model.named_parameters()) if name.find('weight') != -1]
    ...
    if len(weight_params) == 19:
        ...
        tuned_lrs = [
            ...
            {'params': down5_weights,   'lr': lr *
                100, 'weight_decay': weight_decay},
            {'params': down5_bias,      'lr': lr*200, 'weight_upecay': 0.},
            {'params': up1_5_weights,    'lr': lr *
                0.01, 'weight_decay': weight_decay},
            {'params': up1_5_bias,       'lr': lr*0.02, 'weight_decay': 0.},
            {'params': fuse_weights,    'lr': lr *
                0.001, 'weight_decay': weight_decay},
